...
// int main() {
//     const char* filepath = "/home/ubuntu/lib/vcs-ajiant-edr/test_environment/attack_test/test_file_vcs1.txt";
...
//     char* map = (char*) mmap(nullptr, filesize, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
//     if (map == MAP_FAILED) {
//         perror("mmap");
//         close(fd);
//         return 1;
...
//     std::cout << "Original file content:\n" << std::string(map, filesize) << "\n";

//     const char* malicious_data = "ATTACKED_BY_MMAP\n";
//     size_t len = strlen(malicious_data);
//     if (len > filesize) len = filesize;

//     memcpy(map, malicious_data, len);

//     if (msync(map, filesize, MS_SYNC) < 0) {
//         perror("msync");
//     }

//     std::cout << "Modified content written via mmap.\n";
