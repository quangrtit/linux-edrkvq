...
static bool path_has_bad_substr(const std::string &p) {
    static const std::vector<std::string> bad = {"/tmp/", "/var/tmp/", "/dev/shm/"};
...
static bool is_suspicious_library(const std::string &lib) {
    ...
    if (lib.find("backdoor") != std::string::npos) return true;
    return false;
}
...
ScanResult quick_scan_elf(const std::string &path) {
    ...
    if (fd < 0) {
        add_reason(res, "Cannot open file", 0);
        return res;
    }
    ...
    if (ei_class == ELFCLASS64) {
        ...
        // Bỏ check ET_DYN - hầu hết các binary hiện đại đều là PIE
        ...
        if (eh.e_phoff == 0 || eh.e_phnum == 0) {
            ...
            if (ph_size > 10*1024*1024) {
                add_reason(res, "Program header table very large", 5);
            } else {
                ...
                if (!pht_opt) {
                    ...
                    for (size_t i = 0; i < eh.e_phnum; ++i) {
                        ...
                        // W+X segment - dấu hiệu rõ ràng của malware
                        if ((ph.p_flags & (PF_W|PF_X)) == (PF_W|PF_X)) {
                            add_reason(res, "W+X segment detected", 40);
                        }
                        
                        // Executable stack
                        if (ph.p_type == PT_GNU_STACK && (ph.p_flags & PF_X)) {
                            add_reason(res, "Executable stack (no NX)", 35);
                        }
                        
                        // PT_INTERP suspicious
                        if (ph.p_type == PT_INTERP) {
                            auto s = read_string_at(fd, ph.p_offset, 4096);
                            if (s) {
                                std::string interp = *s;
                                if (path_has_bad_substr(interp)) {
                                    add_reason(res, "PT_INTERP in suspicious path: " + interp, 40);
                                }
                                // Bỏ check whitelist - quá nhiều variant hợp lệ
                            }
                        }
                    }

                    // Check DT_NEEDED và RPATH/RUNPATH
                    for (auto &ph : phdrs) {
                        if (ph.p_type == PT_DYNAMIC) {
                            size_t dyn_size = ph.p_filesz;
                            if (dyn_size > 5*1024*1024) continue;
                            auto dyn_raw_opt = pread_all(fd, ph.p_offset, dyn_size);
                            if (!dyn_raw_opt) continue;
                            ...
                            if (dynstr_file_offset != 0 && dyn_str_size > 0) {
                                ...
                                if (dynstr_opt) {
                                    ...
                                    for (auto offset : needed_offsets) {
                                        if (offset < dynstr.size()) {
                                            ...
                                            if (is_suspicious_library(lib)) {
                                                add_reason(res, "Suspicious DT_NEEDED: " + lib, 40);
                                            }
                                        }
                                    }
                                    // Check RPATH/RUNPATH
                                    ...
                                    if (all.find("/tmp/") != std::string::npos || 
                                        all.find("/dev/shm/") != std::string::npos) {
                                        add_reason(res, "RPATH/RUNPATH contains suspicious path", 40);
                                    }
                                }
                            }
                        }
        ...
        // Check sections - chỉ flag những thứ thực sự đáng ngờ
        if (eh.e_shoff != 0 && eh.e_shnum > 0) {
            ...
            if (sht_size < 100*1024) {
                ...
                if (sht_opt && eh.e_shstrndx < eh.e_shnum) {
                    Elf64_Shdr shstr;
                    ...
                    if (shstr_file_off != 0 && shstr_size > 0 && shstr_size < 10*1024*1024) {
                        ...
                        if (shstr_opt) {
                            ...
                            for (size_t i = 0; i < eh.e_shnum; ++i) {
                                ...
                                if (!name.empty()) {
                                    // Section name đáng ngờ
                                    if (name.find(".evil") != std::string::npos || 
                                        name.find("backdoor") != std::string::npos ||
                                        name.find("payload") != std::string::npos) {
                                        add_reason(res, "Suspicious section name: " + name, 45);
                                    }
                                    
                                    // Chỉ check entropy cho .text
                                    if (name == ".text" && (sh.sh_flags & SHF_EXECINSTR)) {
                                        size_t sample_size = std::min<size_t>(2048, sh.sh_size);
                                        auto text_opt = pread_all(fd, sh.sh_offset, sample_size);
                                        if (text_opt) {
                                            double ent = calculate_entropy(text_opt->data(), sample_size);
                                            // Tăng threshold lên 7.0 - chỉ flag packer rõ ràng
                                            if (ent > 7.0) {
                                                add_reason(res, "Very high entropy in .text - likely packed", 30);
                                            }
                                        }
                                    }
                                }
        ...
        // Similar logic cho 32-bit, giữ đơn giản
            ...
            if (ph_size < 10*1024*1024) {
                ...
                if (pht_opt) {
                    for (size_t i = 0; i < eh.e_phnum; ++i) {
                        ...
                        if ((ph.p_flags & (PF_W|PF_X)) == (PF_W|PF_X)) {
                            add_reason(res, "W+X segment (32-bit)", 40);
                        }
                        if (ph.p_type == PT_GNU_STACK && (ph.p_flags & PF_X)) {
                            add_reason(res, "Executable stack (32-bit)", 35);
                        }
                        if (ph.p_type == PT_INTERP) {
                            auto s = read_string_at(fd, ph.p_offset, 4096);
                            if (s && path_has_bad_substr(*s)) {
                                add_reason(res, "Suspicious PT_INTERP (32-bit)", 40);
                            }
                        }
                    }
                }
        ...
        add_reason(res, "Unknown ELF class", 5);
        close(fd); 
        return res;
    }
}
